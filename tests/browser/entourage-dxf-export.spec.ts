...
test('built-in entourage-only plans export editable DXF linework',async({page},testInfo)=>{
 ...
 floor.entourage=['person','car-sedan','tree-deciduous','hedge'].map((defId,i)=>({id:`e${i}`,defId,position:{x:(i%2)*600-500,y:Math.floor(i/2)*600-500},width:300,rotation:i===1?35:0}));
 ...
 const dxf=await download('Export as DXF');expect(dxf).toContain('ENTOURAGE');expect(dxf).toContain('SPLINE');expect(dxf).not.toMatch(/NaN|Infinity/);
