...
test('exports include oversized rotated furniture outside the wall bounds',async({page},testInfo)=>{
 ...
 floor.furniture=[{id:'large',catalogId:'unknown',position:{x:-600,y:-600},rotation:45,width:800,depth:300,color:'#e00000',scale:{x:1,y:1,z:1}}];
 ...
 const size=await page.evaluate(svg=>{
  ...
  const rect=doc.querySelector('rect[width="800"]')!, transform=rect.parentElement!.getAttribute('transform')!.match(/-?\d+(?:\.\d+)?/g)!.map(Number);
  const angle=transform[2]*Math.PI/180;
  const points=[-400,400].flatMap(x=>[-150,150].map(y=>({x:transform[0]+x*Math.cos(angle)-y*Math.sin(angle),y:transform[1]+x*Math.sin(angle)+y*Math.cos(angle)})));
