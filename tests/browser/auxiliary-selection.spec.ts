...
for(const [key,label] of [['guides','horizontal guide 1'],['measurements','Measurement 1'],['annotations','Annotation 1']]) {
  test(`${key} selection clears on Escape and deletes only its target`,async({page})=>{
    ...
    async function select(){await save.press('l');await page.getByRole('button',{name:new RegExp(label)}).click();await save.press('l');}
    ...
    await select();await save.press('Delete');const deleted=await exported();expect(deleted[key]).toEqual([]);
    for(const other of ['walls','doors','windows','guides','measurements','annotations'].filter(other=>other!==key))expect(deleted[other]).toEqual(before[other]);
